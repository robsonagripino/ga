...
public class GeneticAlgorithm {
    ...
    public double calcFitness(Individual individual) {
        ...
        for(int i = 0; i < individual.getChromosomeLength(); i++){
            ind  += Integer.toString(individual.getGene(i));
        }
        
        int angulo1 = Integer.parseInt(ind.substring(0,8), 2);
        int angulo2 = Integer.parseInt(ind.substring(8,16), 2);
        
        double rad1 = (Math.PI/180) * angulo1;
        double rad2 = (Math.PI/180) * (angulo1+angulo2);
        
        int l1 = 10;//Integer.parseInt(ind.substring(0, 10), 2);
        int l2 = 10;//Integer.parseInt(ind.substring(18, 28), 2);
        
        double x = l1*Math.cos(rad1) + l2*Math.cos(rad2);
        double y = l1*Math.sin(rad1) + l2*Math.sin(rad2);
        
        double objetivoX, objetivoY;
        objetivoX = 20;
        objetivoY = 20;
        
        double dist = Math.sqrt((x-objetivoX)*(x-objetivoX)+(y-objetivoY)*(y-objetivoY));
       // double fitX = 0.5*(1/(1+Math.abs(x-objetivoX)));
       // double fitY = 0.5*(1/(1+Math.abs(y-objetivoY)));
        //System.out.println("X = "+ x+" fitx = "+fitX);
        double fitDist = 1/(1+dist);     
        //double fitness = fitX+fitY;
        individual.setFitness(fitDist);
        return fitDist;
    }
    ...
    public Population crossoverPopulation(Population population) {
        ...
        for (int populationIndex = 0; populationIndex < population.size(); populationIndex++) {
            ...
            if (this.crossoverRate > Math.random() && populationIndex >= this.elitismCount) {
                ...
                int pontoCorte = (int) (Math.random() * parent1.getChromosomeLength());
                ...
                //System.out.println("Cruzou "+ parent1.toString() +" com "+ parent2.toString()+ "   "+pontoCorte +" = "+ offspring.toString() );
                // Loop over genome
                /*for (int geneIndex = 0; geneIndex < parent1.getChromosomeLength(); geneIndex++) {
                    // Use half of parent1's genes and half of parent2's genes
                    if (0.5 > Math.random()) {
